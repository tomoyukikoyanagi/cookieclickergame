...
//  ACTGame
//
//  Created by TomoyukiKoyanagi on 2020/04/14.
...
class skMenu: SKNode, SKPopMenuDelegate {
    
    var popMenuDelegate: SKPopMenuDelegate?
    var numberOfSections: Int!
    let defaultColor = SKColor.black
      ...
      for touch in touches {
        for node in sections {
          if node.contains(touch.location(in: self)) {
            self.popMenuDelegate?.sectionTapped!(sections.firstIndex(of: node)!, name: node.name!)
          }
        }
      }
    }
